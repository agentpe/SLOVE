...
def find_slove(explicit):
    ...
    for root in roots:
        pscs = (
            os.path.join(root, "papyrus", "Source", "SLOVE_Voice.psc"),
        ...
        tomls = (
            ...
            os.path.join(
                root, "SKSE", "Plugins", "AudioUtil", "config", "SLOVE_voices.toml"
            ),
        )
        ...
        if psc and toml:
            return psc, toml
    sys.exit(
        "could not find SLOVE_Voice.psc + SLOVE_voices.toml.\n"
...
def find_pack(path):
    ...
    ids, found = {}, {}
    overlays = glob.glob(
        os.path.join(path, "**", "SKSE", "Plugins", "AudioUtil", "config", "*.toml"),
        recursive=True,
    )
    for overlay in sorted(overlays):
        ...
        for slot in cfg.get("slot", []):
            ...
            if folder:
                ids.setdefault(folder, {})[slot.get("id", "?")] = norm(
                    slot.get("fallback") or ""
                )
...
def parse_calls(psc_path):
    ...
    calls = []
    pattern = re.compile(
        r'PlaySound\(\s*"([^"]+)"\s*,\s*([A-Za-z_]\w*)(.*?)\)\s*(?:;|$)', re.M
    )
    for match in pattern.finditer(src):
        ...
        female = "forceFemaleVoice" in rest or (
            actor == "mainFemaleActor" and not voice_actor
        )
        if female:
            calls.append(
                (
...
def load_maps(toml_path):
    ...
    with open(toml_path, "rb") as handle:
        cfg = tomllib.load(handle)
    aliases = {
        norm(k): v for k, v in cfg.get("category_aliases", {}).get("female", {}).items()
    }
    fallbacks = {
        norm(k): v
        for k, v in cfg.get("category_fallbacks", {}).get("female", {}).items()
    }
    slots = {}
    for slot in cfg.get("slot", []):
...
def main():
    ...
    parser.add_argument(
        "pack", help="your pack's root folder, or its Sound\\fx\\SLOVE\\<Name> folder"
    )
    parser.add_argument(
        "--slove", help="an installed SLO VE folder (default: this repo)"
    )
    parser.add_argument(
        "--slot",
    ...
    if chosen not in slot_fallbacks:
        sys.exit(
            "this pack ships no slot {!r} - it offers: {}".format(
    ...
    print(
        "SLO VE pack check - as slot {} (backs onto {}), variation {}".format(
            chosen, fallback_slot.upper() or "nothing", variation
        )
    )
    if others:
        print(
            "  this pack also installs as {} - re-run with --slot to check those".format(
                ", ".join(others)
            )
        )
    print("  " + voice_folder)
    ...
    if mismatches:
        ...
        print(
            "SUSPECTED NAME MISMATCH ({}) - check each of these".format(len(mismatches))
        )
        print(
            "  SLO VE asks for a name your pack does not use, while you ship a folder"
        )
        ...
        for category, lines, suggestion in mismatches:
            ...
            print(
                "        you ship '{}' - plays {} instead".format(
                    suggestion, went_to[category]
                )
            )
    ...
    dead = [
        (display, count)
        for key, (display, count) in sorted(
            folders.items(), key=lambda kv: kv[1][0].lower()
        )
    ...
    if plain:
        print(
            "NOT VOICED BY THIS PACK ({}) - normal, you ship nothing close".format(
                len(plain)
            )
        )
        if args.all:
