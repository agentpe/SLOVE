...
Function RegisterForTheEventsWeNeed()
	...
	;(RegisterForSingleUpdate dies on save/load and can be starved by script lag),
	;so end on SexLab's own AnimationEnd too - see DirectorSceneEnd
...
Event DirectorSceneStart(string eventName, string argString, float argNum, form sender)
	...
	;scenes), latching PlayerInScene true forever - the old guard only cleared it
	;when NO thread existed, so at a real new scene it never cleared and every
	;future scene was ignored. Re-adopting on a thread-id change is the safety net.
	if PlayerInScene
		if CurrentThread && newThread.GetThreadID() == CurrentThreadID
...
Function AdoptScene()
	...
	CurrentSceneName = SexlabRegistry.GetSceneName(CurrentSceneID)
...
;AnimationEnd instead of waiting for the OnUpdate poll to notice the thread is
;gone (the poll can be dropped by save/load or script lag, stranding
...
;================= CONSOLE DIAGNOSTIC: current-anim dump =====================
;Dumps the live scene's tags, per-actor labels, resolved AudioUtil slot and the
;label-derived voice branch. Each line goes to BOTH the console AND the SLOVE user
;log (SLOVE.0.log) via DumpLine, so it is visible in-game and captured for later
;reading. Reached from SLOVE_Test.DumpAnim (the `slovetest anim` console command) -
;a user-invoked command, NOT the load path (console printing on the thaw CTDs the load).
Function DumpCurrentAnim()
	...
	DumpLine("=== SLO VE anim dump ===")
	DumpLine("scene " + currentSceneID + "  stage " + CurrentStageNum + "/" + GetStagesCount() + "  intense=" + SceneisIntense() + "  time=" + (GetTimeTotal() as int) + "s")
	DumpLine("tags: " + DebugPresentTags())
	DumpLine("SFX tag: " + DebugSfxLabel(SLOVE_Hentairim_Tags.GetSFX(currentSceneID, CurrentStageNum)))
...
;Label-derived voice branch for one actor, mirroring the SLOVE_Voice.OnUpdate predicate
;ORDER. Approximate: OnUpdate also has gag/orgasm-hype/timing branches that this cannot
;see from labels alone. [] shows the representative category for that branch where known.
