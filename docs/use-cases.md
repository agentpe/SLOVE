...
| B11 | **Cunnilingus detection + contact tongue (P+)** | P+ cunnilingus scene (female oral target), `enabletongue=1` | SexLab's legacy detector reports clit↔mouth contact as `aOral` → the `CUN` oral label, and the licker gets a contact tongue. Sensitivity is tuned in **`SexLab.ini`** (`fDistanceMouth`, `fAngleCunnilingus`) — not from SLO VE; see [Tuning Cunnilingus Detection](cunnilingus-detection.md) | Needs `bUseLegacyNiType=1` + a body with a `Clitoral1` node. `CUN` appears in the physics-label debug (`director.printdebug`); tongue = user visual. (The `director.cunnilingusdistance/angle` TOML keys are inert — the `sslSystemConfig` path they used never worked.) |
