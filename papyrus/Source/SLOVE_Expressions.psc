...
;The oral-tongue trigger: authored LABEL or live physics - either qualifies.
; - label:   CUN (cunnilingus, per-stage/per-actor) - reliable on its own, and
...
Function HentairimUpdateStageData()
	...
	if stagechanged
		...
		;can never disagree: lick (tag gate + live P+ proximity flags), intense
		;receiving, and attacking positions. The chance rolls only gate the ADD
		bool oralTongue = IsOralTongueActive() && cunusetongue == 1
