...
Function InitializeConfigValues()
	...
	ActorsInPlay = currentthread.Positions
...
Function PerformInitialization()
	...
	timeOfLastStageStart = currentthread.TotalTime
...
Function RecordFemaleOrgasm()
	...
	timeOfLastRecordedFemaleOrgasm = currentthread.TotalTime
...
Bool Function ShouldMakeRomanticComment()
	if femaleisvictim()
		return false
	elseIf currentthread.TotalTime - timeOfLastRomanticRemark < 60 ;Too soon. Romantic comments should be spaced out and rare
		Return False
...
Bool Function FemaleIsVictim()
	return currentthread.IsVictim(mainFemaleActor) && !ASLisBroken() && EnableVictimScenario == 1
EndFunction

Bool Function MaleIsVictim()
	return currentthread.IsVictim(mainMaleActor) && EnableVictimScenario == 1
EndFunction
...
Function ASLPlayFemaleOrgasmHype()
	...
	if currentthread.TotalTime - timeOfLastRecordedFemaleOrgasm <= 8
		CommentedClosetoOrgasm = true
...
Bool Function IsfinalStage()
	return currentstage == GetLegacyStagesCount("")
endfunction
...
;classic: the stage count is the active animation's StageCount
